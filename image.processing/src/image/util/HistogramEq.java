...
public class HistogramEq {
	public static int[][] getRGB(File file) throws IOException {
	...
	public static float[] RGB2GS(File file) throws IOException {
		BufferedImage buf = ImageIO.read(file);
		int width = buf.getWidth();
		int height = buf.getHeight();
		int size = width * height;
	...
	public static int[] histogram(float[] grayScale) {
	...
	public static int[] getCDF(int[] histogram) {
		int[] cdf = new int[256];
		int cum = 0;
		for (int i = 0; i < 256; i++) {
			cum += histogram[i];
	...
	public static int getMinCDF(int[] cdf) {
		int minCDF = 257;
		for (int i = 0; i < 256; i++) {
	...
	public static int getMaxCDF(int[] cdf) {
		int maxCDF = 0;
		for (int i = 0; i < 256; i++) {
			if (cdf[i] > maxCDF)
				maxCDF = cdf[i];
	...
	public static int[] floatToInt(float[] cdf) {
		int length = cdf.length;
		int[] newCDF = new int[length];
	...
	public static float[] equalization(int[] cdf, int pictSize) {
...
//		System.out.println("pictSize: " + pictSize);
	...
	public static float[] picEqualized(float[] grayScale, float[] equalization, int w, int h) {
		int size = w * h;
	...
	public void drawImage(float[] newGS, int w, int h) throws IOException {
		...
		ImageIO.write(im, "PNG", new File("data/piscines-eq.png"));
	}

	public static void main(String args[]) throws IOException {
		HistogramEq he = new HistogramEq();
		File file = new File("data/piscines-gray.png");
